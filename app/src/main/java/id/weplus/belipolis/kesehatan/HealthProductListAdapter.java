...
public class HealthProductListAdapter extends RecyclerView.Adapter<HealthProductListAdapter.ProductViewHolder>
        implements Filterable {
    private Activity activity;
    private int catId;
    private List<Product> products;
    private List<Product> originalProduct;
    private HealthProductListRequest request;
    private HealthProductListAdapter.ItemFilter mItemFilter = new HealthProductListAdapter.ItemFilter();


    public HealthProductListAdapter(Activity activity, List<Product> productList, int catId, HealthProductListRequest healthRequest) {
        this.activity = activity;
        this.products = productList;
        this.catId=catId;
        this.originalProduct=productList;
        this.request = healthRequest;
    }

    @Override
    public HealthProductListAdapter.ProductViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View childView = LayoutInflater.from(activity).inflate(R.layout.viewadapter_produk_terbaru, viewGroup, false);
        return new HealthProductListAdapter.ProductViewHolder(childView);
    }

    @SuppressLint("SetTextI18n")
    @Override
    public void onBindViewHolder(HealthProductListAdapter.ProductViewHolder holder, int i) {
        ...
        holder.btnPilih.setOnClickListener(view -> {
            ...
            Intent intent = new Intent(activity, FormBuyHealth.class);
            intent.putExtra("product_detail", prod);
            intent.putExtra("request_body", request);
            intent.putExtra("cat_id", 2);
            activity.startActivity(intent);
