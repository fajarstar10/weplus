...
public class TravelListAdapter extends RecyclerView.Adapter<TravelListAdapter.ProductViewHolder>
        implements Filterable {
    private Activity activity;
    private int catId;
    private List<Product> products;
    private List<Product> originalProduct;
    private TravelProductListRequest request;
    private TravelListAdapter.ItemFilter mItemFilter = new TravelListAdapter.ItemFilter();


    public TravelListAdapter(Activity activity, List<Product> productList, int catId, TravelProductListRequest travelRequest) {
    ...
    @Override
    public TravelListAdapter.ProductViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View childView = LayoutInflater.from(activity).inflate(R.layout.viewadapter_produk_terbaru, viewGroup, false);
        return new TravelListAdapter.ProductViewHolder(childView);
    }

    @SuppressLint("SetTextI18n")
    @Override
    public void onBindViewHolder(TravelListAdapter.ProductViewHolder holder, int i) {
        ...
        holder.btnPilih.setOnClickListener(view -> {
            Intent intent = new Intent(activity, FormBuyTravel.class);
            intent.putExtra("product_detail", prod);
            intent.putExtra("request_body", request);
            intent.putExtra("cat_id", 7);
            activity.startActivity(intent);
